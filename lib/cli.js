...
cli.responders.help = function(){
  ...
  let commands = {
    ...
    'list logs':'Show a list of all the log files available ot be read (compressed)',
  ...
  //Shos each command, followed by its explanation in white and yellow respectively
  ...
  //End with enother horizontal line
  cli.horizontalLine();
    ...
    if(!err && checkIds && checkIds.length > 0){
      ...
      checkIds.forEach(function(checkId){
        _data.read('checks',checkId,function(err,checkData){
          ...
          let stateOrUnknown = typeof(checkData.state) == 'string' ? checkData.state : 'unknow';
          //If the user has specified the state, or hasn`t specified any state inculding current check accordingly
...
cli.responders.listLogs = function(){
  ...
  ls.stdout.on('data', function(dataObj){
    //Explode onto  separate lines
    let dataStr = dataObj.toString();
    let logFileNames = dataStr.split('\n');
    cli.verticalSpace();
    logFileNames.forEach(function(logFileName){
    // console.log(logFileName);   
    if(typeof(logFileName) == 'string' && logFileName.length > 0 && logFileName.indexOf('-') > -1){ // That check did not work correctly
...
const cliM = 'cli: '
// const cliMsg = `\x1b[33m` + `${cli}` + `\x1b[0m`  
